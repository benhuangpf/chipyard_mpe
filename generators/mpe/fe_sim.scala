...
class Frontend(numTree: Int) extends MultiIOModule {
  ...
  //  00             01              10              11
  val cpuSinkIdle :: cpuSinkWBusy :: cpuSinkBValid :: cpuSinkIssue :: cpuSinkB :: Nil = Enum(5)
  ...
  val axrw    = Reg(Bool())
  io.cpu.aw.ready := (cpuSinkState === cpuSinkIdle || cpuSinkState === cpuSinkWBusy) & io.cpu.aw.valid
  io.cpu.ar.ready := !io.cpu.aw.ready & (cpuSinkState === cpuSinkIdle || cpuSinkState === cpuSinkWBusy) & io.cpu.ar.valid
  ...
  io.cpu.r.valid     := (rdataIdx === 7.U(3.W))
  ...
  io.cpu.b.valid     := (cpuSinkState === cpuSinkBValid) | (cpuSinkState === cpuSinkB)
  ...
  when (cpuSinkState === cpuSinkWBusy) {
    ...
    when (io.cpu.w.valid) {
      ...
      // cpuSinkState := cpuSinkBValid
      cpuSinkState := Mux(wlast, cpuSinkIssue, cpuSinkBValid)

      printf_dbg("  Frontend::cpuSink::W / wdata64x8[%d]: %x\n", wdataIdx, wdata_t)
    }

  }
  when (cpuSinkState === cpuSinkBValid) {
    // cpuSinkState := Mux(wlast, cpuSinkIssue, cpuSinkWBusy)
    cpuSinkState := cpuSinkWBusy
  }
